...
class MinecraftQuery:
    ...
    def __init__(self, host, port, timeout=10, id=0, retries=2):
        self.addr = (host, port)
        self.id = id
        self.id_packed = _struct.pack('>l', id)
        self.challenge_packed = _struct.pack('>l', 0)
        self.retries = 0
    ...
    def send_packet(self, type, data=''):
        self.send_raw(_struct.pack('>B', type) + self.id_packed + self.challenge_packed + data)
    
    def read_packet(self):
        buff = self.socket.recvfrom(1460)[0]
        type = _struct.unpack('>B', buff[0])[0]
        id = _struct.unpack('>l', buff[1:5])[0]
    ...
    def handshake(self, bypass_retries=False):
        ...
        self.challenge_packed = _struct.pack('>l', self.challenge)
    ...
    def get_status(self):
        ...
        data['hostport'] = _struct.unpack('<h', buff[:2])[0]
        buff = buff[2:]
